...
void SeqListDeletePos(SeqList *psl, DataType pos);
void SeqListDeleteVal(SeqList *psl, DataType val);
void SeqListFindPos(SeqList *psl, DataType pos);
void SeqListFindVal(SeqList *psl, DataType val);
...
void SeqListDeletePos(SeqList *psl, DataType pos)
{
	...
	else
	{
		if ((psl->size) > pos)
		{
			...
			{
				psl->base[pos+i] = psl->base[pos + i+1];
			}
			psl->size--;
		}
